...
wire SD_DMA_TO_ROM;
// Bus-timing compat toggle (featurebits[13] = CFG.bus_compat, reuses the dead
// FEAT_COMBO bit).  Compat=1 restores the 56dd166-reverted pulse-end-strobe time
// sharing: releases the cart databus EARLIER (SNES_PULSE_end / SNES_READ_narrow) ->
// avoids bus contention on timing-sensitive 1-CHIP consoles (= v1.11.0 behavior, fixes games
// like DKC that hang on those units).  Standard=0 (default) keeps the current wide
// window (SNES_cycle_end / SNES_READ) that the reverting units need.
wire bus_compat    = featurebits[13];
wire free_end_sel  = bus_compat ? SNES_PULSE_end  : SNES_cycle_end;
wire SNES_READ_sel = bus_compat ? SNES_READ_narrow : SNES_READ;
wire free_slot = (free_end_sel | free_strobe) & ~SD_DMA_TO_ROM;
...
mcu_cmd snes_mcu_cmd(
  ...
  .saveram_mask_b_out(SAVERAM_MASK_B),
  .SD_DMA_EN(SD_DMA_EN),
  .SD_DMA_STATUS(SD_DMA_STATUS),
  .SD_DMA_NEXTADDR(SD_DMA_NEXTADDR),
  .SD_DMA_SRAM_DATA(SD_DMA_SRAM_DATA),
...
/* Bootleg copy-protection (dedicated core, see bootleg.v).  The MCU picks the
   variant from the ROM CRC32 and sends it as chipfeat[2:0] (CMD 0xef) before the
   SNES leaves reset. */
wire       bootleg_rd_hit;
...
assign SNES_DATABUS_OE = bootleg_open_bus ? 1'b1 :   // protection "open bus" windows
                         ...
                         (snescmd_enable & ReadOrWrite_r) ? (~(snescmd_unlock | feat_cmd_unlock | (map_snescmd_wr_unlock_r & ~SNES_WRITE) | (map_snescmd_rd_unlock_r & ~SNES_READ_sel))) :
                         (bs_page_enable & ~SNES_READ_sel) ? 1'b0 :
