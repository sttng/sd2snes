...
module LoopyGen(input clk, input ce,
  ...
  assign loopy_t_out = loopy_t;
  // Write toggle (the shared $2005/$2006 first/second-write latch; 2A03 "w"):
  // 0 = no pending pair (a $2005/$2006 write pair is COMPLETE), 1 = mid-pair
  // (only the first byte written, loopy_t holds an INTERMEDIATE value).  The
  // split capture uses this to finalize an entry only on a complete pair, so
  // the intermediate-T of a scanline-straddling pair never becomes an entry.
  assign w_out = ppu_address_latch;
endmodule
...
module SpriteRAM(input clk, input ce,
                 ...
                 output [7:0] oam_ptr_out);  // sd2snes bridge tap: current OAM write pointer
  reg [7:0] sprtemp[0:31];   // Sprite Temporary Memory. 32 bytes.
  reg [7:0] oam[0:255];      // Sprite OAM. 256 bytes.
  reg [7:0] oam_ptr;         // Pointer into oam_ptr.
  assign oam_ptr_out = oam_ptr;   // sd2snes bridge tap
...
module PPU(input clk, input ce, input reset,   // input clock  21.48 MHz / 4. 1 clock cycle = 1 pixel
           ...
           // sd2snes video-bridge Class B taps (NES-BRIDGE-SPEC.md SS2.2; molde dbg_cpu).
           // ALL REGISTERED at the output -- see the "registered output stage"
           // block at the end of this module for the timing-isolation rationale
           // (this cost an STA run; do not export raw combinational taps).  The
           // we strobes come out ALREADY ce-qualified as clean 1-CLK2 pulses;
           // consumers must NOT re-gate them with ce.
           output reg        tap_pal_we,    // 1-CLK2 pulse, 1 per palette write
           output reg [4:0]  tap_pal_idx,   // raw palette index (bridge applies $3F1x mirror)
           output reg [5:0]  tap_pal_val,
           output reg        tap_oam_we,    // 1-CLK2 pulse, 1 per OAM byte write
           output reg [7:0]  tap_oam_addr,
           output reg [7:0]  tap_oam_val,
           output reg [14:0] tap_loopy_t,   // scroll intent (NOT loopy_v -- see nes_bridge)
  ...
  // sd2snes bridge taps: full raw $2000/$2001 bytes (the split fields above lose
  // bits 0-1/6 of ctrl; the bridge needs the whole byte, matching ppustate).
  reg [7:0] ppuctrl_full, ppumask_full;
  ...
  wire [7:0] oam_bus;
  wire [7:0] oam_ptr_tap;   // sd2snes bridge tap (registered into tap_oam_addr below)
...
//      $write("%d/%d: $200%d <= %x\n", scanline, cycle, ain, din);
    if (write) begin
      ...
      // t:....BA.. ........ = d:......BA
        ppuctrl_full <= din;   // sd2snes bridge tap
        obj_patt <= din[3];
        bg_patt <= din[4];
        obj_size <= din[5];
        vbl_enable <= din[7];
      ...
      1: begin // PPU Control Register 2
        ppumask_full <= din;   // sd2snes bridge tap
        grayscale <= din[0];
  ...
  // ---- sd2snes video-bridge Class B taps: REGISTERED OUTPUT STAGE ----
  // TIMING ISOLATION -- this cost the first STA run (setup -10.4ns, TNS -4009;
  // do not regress by exporting raw combinational taps): `din` is NOT a
  // CPU-only net.  Through the $2007 palette READBACK path it is combination-
  // ally reachable from the RENDER pixel pipe -- Sprite x_coord -> pixel mux
  // -> PaletteRam ASYNC read -> color -> latched_dout/ppu_dout ->
  // from_data_bus -> CPU -> cpu_dout -> dbus -> din: a ~22.5ns cone.  INSIDE
  // the NES that cone is legal (every consumer is ce-gated, covered by
  // main.sdc's {*|NES:core|*} setup-4 multicycle, ce spacing >= 13 CLK2);
  // exporting it RAW into the bridge's free-running registers escaped that
  // umbrella and violated single-cycle setup (worst paths x_coord -> pal~*).
  // Registering every tap HERE terminates the deep cone in NES-domain
  // registers (auto-matched by the {*|NES:core|*} multicycle patterns):
  //   * data regs are ce-gated -> plain setup-4 destinations, like every
  //     other NES register (e.g. PaletteRam itself, same din);
  //   * the we strobes sample every clock BUT can only assert on the ce
  //     cycle, and their non-ce inputs (write/ain/is_pal_address) are launched
  //     >= 13 CLK2 before that ce -> the setup-4 relaxation on those paths is
  //     safe (same argument as the wrap-FSM setup-2 boundary, main.sdc #2);
  //     `ce` itself comes from nes_wrap's ce_pulse_r (outside NES:core) ->
  //     stays single-cycle, as it must.
  // The exported cones are then exactly ONE register each.  Strobes are clean
  // 1-CLK2 pulses, ALREADY ce-qualified (consumers must not re-gate with ce).
  // Pairing: on a write ce at edge E, tap_*_we <= 1 and the if(ce) data regs
  // capture the PRE-update values (loopy/oam_ptr before their own increment,
  // as NBA reads the old value) -> the (we,idx/addr,val) triple is coherent
  // from cycle E+1 on, exactly what the direct wires used to show during E.
  always @(posedge clk) begin
    tap_pal_we <= ce && write && (ain == 7) && is_pal_address;
    // OAM: $2004 store; addr = oam_ptr (pre-increment), raw din (SpriteRAM
    // masks attr bits 2-4 in its own array; renderer ignores those bits).
    tap_oam_we <= ce && write && (ain == 4);
    if (ce) begin
      tap_pal_idx  <= loopy[4:0];
      tap_pal_val  <= din[5:0];
      tap_oam_addr <= oam_ptr_tap;
      tap_oam_val  <= din;
    end
    // snapshot levels: D cones are pure register outputs (loopy_t / loopy_x /
    // ppuctrl_full / ppumask_full) -> every-clock re-register is a wire path.
    tap_loopy_t <= loopy_t_tap;
    tap_fine_x  <= fine_x_scroll;
