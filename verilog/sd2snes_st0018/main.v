...
// ST018 (see the st018 instance below)
wire [7:0] ST018_SNES_DATA_OUT;
wire       st018_enable;
wire       st018_ld_reset, st018_ld_we, st018_ld_busy;
wire [7:0] st018_ld_data;
...
//
// It costs 8 BRAM on the mk2 XC3S400 and 16 M9K on mk3, and no ST018
// cart uses it. (Inherited from the ST011 core this was derived from.)
//
...
// ---------------------------------------------------------------------------
// ctx.v (WRAM/APU context shadow) is not instantiated in this core.
//
// It mirrors every WRAM/APU write into PSRAM for full savestates, and
// savestate.c never enables savestates or the in-game handler on a core
// outside its core_has_snapshot list -- ST018 is not on it. On the mk2
// XC3S400 it is 869 LUTs that the ARM core needs. Its outputs are tied off
// so the PSRAM arbiter branches that serve it constant-fold away.
// ---------------------------------------------------------------------------
wire [23:0] CTX_ADDR = 24'h000000;
wire [15:0] CTX_DOUT = 16'h0000;
wire CTX_WRQ  = 1'b0;
wire CTX_WORD = 1'b0;
wire CTX_DBG  = 1'b0;
wire CTX_RDY;
wire ctx_wr_enable   = 1'b0;
wire ctx_pawr_enable = 1'b0;
wire ctx_pard_enable = 1'b0;

// ---------------------------------------------------------------------------
// dma.v (the $2020 copier) is not instantiated in this core.
//
// Its only user is the savestate/overlay handler code, which never runs on
// this core (see ctx above); 475 LUTs on the mk2. address.v's dma_enable is
// tied low for the same reason, so $2020-$202F reads are ordinary open bus.
// ---------------------------------------------------------------------------
wire [23:0] DMA_ADDR = 24'h000000;
wire [15:0] DMA_DOUT = 16'h0000;
reg  [15:0] DMA_DINr;
wire DMA_RRQ = 1'b0;
wire DMA_WRQ = 1'b0;
wire DMA_WORD = 1'b0;
wire DMA_LOOP_ENABLE = 1'b0;
wire DMA_RDY;
assign DMA_SNES_DATA_OUT = 8'h00;
...
// ---------------------------------------------------------------------------
// Seta ST018: ARMv3 coprocessor (st018.v / st018_cpu.v)
//
// Host registers at $00-3F/$80-BF:3800-38FF (address.v: st018_enable).
// The 160 KB firmware image (st018.rom: 128 KB program + 32 KB data ROM) is
// streamed by the MCU into the Bus 2 SRAM via $E8/$E9 while the ARM is held
// in reset by $EB, verified with the existing $E5/$F5 checksum sweep, and
// then released. The ARM executes out of an on-chip cache in front of it.
// ---------------------------------------------------------------------------
wire [15:0] dsp_feat;   // mcu_cmd.v output (event-board timer settings)
...
// unused, tied off so the arbiter branch constant-folds away.
reg RQ_DSP_RDYr;
...
st018 #(.CACHE_LB(ST018_CACHE_LB)) snes_st018 (
  ...
  .snes_sel(st018_enable),
  .snes_a(SNES_ADDR[2:1]),
  .snes_di(BUS_DATA),
  .snes_do(ST018_SNES_DATA_OUT),
  .snes_rd_start(SNES_RD_start),
  .snes_rd_end(SNES_RD_end),
  .snes_wr_end(SNES_WR_end),
  .ld_reset(st018_ld_reset),
  .ld_we(st018_ld_we),
  .ld_data(st018_ld_data),
  .ld_busy(st018_ld_busy),
  .sum_start(dspx_vsum_start),
  .sum_busy(dspx_vsum_busy),
  .sum(dspx_vsum),
  .RAM_ADDR(RAM_ADDR),
  .RAM_DATA(RAM_DATA),
  .RAM_OE(RAM_OE),
  .RAM_WE(RAM_WE),
  .dbg_retire(), .dbg_pc(), .dbg_ir(), .dbg_rf_we(), .dbg_rf_wa(), .dbg_rf_wd(),
  .dbg_cpsr(), .dbg_miss_cnt()
);

// $3800/$3804 (A1 = 0) drive the bus on reads; $3802/$3806 reads are open
// bus (MesenCE: addr & $FF06). Writes need the buffer enabled either way.
...
mcu_cmd snes_mcu_cmd(
  ...
  .st018_ld_reset_out(st018_ld_reset),
  .st018_ld_we_out(st018_ld_we),
  .st018_ld_data_out(st018_ld_data),
...
assign SNES_DATA = (r213f_enable & ~SNES_PARD) ? (r213f_forceread ? 8'bZ : r213fr)
                   ...
                   :(((~SNES_READ & ((~SNES_SNOOPPAWR_DATA_OE & ~SNES_SNOOPPARD_DATA_OE) | ~SNES_ROMSEL_EARLY)))
                                ...
                                ? (cc_sel ? CC_SR
                                  :st018_enable ? ST018_SNES_DATA_OUT
...
assign MCU_RDY = RQ_MCU_RDYr & ~st018_ld_busy;   // loader flow control (FPGA_WAIT_RDY)
// CTX
reg RQ_CTX_RDYr;
...
// DSP program fetch request (ST011 -- see ST_DSP_RD_* above).
...
assign SNES_DATABUS_OE = ((cc_sel | st018_bus) & ReadOrWrite_r) ? 1'b0 :
